...
EXPECTED_NODES = frozenset({
    "rag", "hypotheses", "instruction", "probe_wait",
    "step", "reason", "decision", "repair", "interrupt", "resume",
})
...
def test_conditional_routes_from_decision_exist():
    ...
    g = create_conversational_graph()
    decision_targets = {t for s, t in
                        ((e.source, e.target) for e in g.get_graph().edges)
                        if s == "decision"}
...
def test_state_schema_carries_required_fields():
    ...
    required = {
        "messages", "equipment_model", "equipment_config", "test_points",
        "expected_values", "hypotheses", "hypothesis_probabilities",
        "eliminated_faults", "current_hypothesis", "test_point_rankings",
        "measurements", "suspected_faults", "confirmed_fault",
        "current_step", "max_steps", "consecutive_failures",
        "pending_manual_reading", "next_node", "step_result",
    }
