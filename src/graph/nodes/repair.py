...
def _record_terminal_case(
    ...
    """Fire-and-forget write of the completed case to the diagnostic_cases table.

    Any failure here is swallowed — the user-visible flow must never break
    because a DB is unavailable.
    """
    try:
...
def repair_node(state: ConversationalAgentState, config: RunnableConfig | None = None):
    ...
    fault_record: dict = {}
    if fault_id:
        fault_record = next((f for f in faults if f.get("fault_id") == fault_id), {})

    # If no direct match, ask LLM to match measurement → fault.
    if not fault_record and faults:
        faults_summary = "\n".join(
            f"- {f.get('fault_id','')}: {f.get('name','')}" for f in faults[:8]
        )
        try:
            ...
            fault_record = next((f for f in faults if f.get("fault_id") == matched_id), {})
        except Exception:
            pass
        ...
        if faults:
            ...
            try:
                ...
                if sec_id and sec_id != "NONE" and sec_id not in seen_fault_ids:
                    sec_record = next((f for f in faults if f.get("fault_id") == sec_id), {})
                    if sec_record:
    ...
    if fault_record.get("recovery"):
        ...
        repair_lines.append(
            "_No repair procedure found in config for root cause. Consult service manual._"
        )
    ...
    for m in state.measurements:
        ...
        evidence_rows.append(
            f"| {m.get('signal_name', m.get('test_point','?'))} " f"| {val} {unit} | {icon} {ev} |"
        )
    ...
    evidence_table = (
        ...
        "|------------|---------|--------|\n" + "\n".join(evidence_rows)
        if evidence_rows
        else "_No measurements recorded._"
    )
    ...
    if output_signal:
        ...
        verify_msg = (
            ...
            if oexpect
            else f"**Post-repair verification:** Confirm **{oname}** is within normal operating range."
        )
    else:
        verify_msg = (
    ...
    _record_terminal_case(state, config, outcome="resolved", final_diagnosis=fault_name)
