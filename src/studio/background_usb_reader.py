...
class RobustStabilizer:
    ...
    NOISE_THRESHOLD_VOLTS = (
        0.1  # matches BackgroundReader.NOISE_THRESHOLD; allows sub-0.5V test points
    )
    ...
    def get_fluctuation_threshold(self) -> float:
        ...
        return self.FLUCTUATION_THRESHOLDS.get(
            self.measurement_type, self.FLUCTUATION_THRESHOLDS["DEFAULT"]
        )
    ...
    def get_absolute_threshold(self) -> float:
        ...
        return self.ABSOLUTE_THRESHOLDS.get(
            self.measurement_type, self.ABSOLUTE_THRESHOLDS["DEFAULT"]
        )
    ...
    def _is_outlier(
        self, value: float, center: float, mad: float, threshold_multiplier: float = 3.0
    ) -> bool:
        """
    ...
    def _find_stable_clusters(self, values: list) -> list[tuple[int, int, float, float]]:
        ...
        for i in range(len(values) - self.MIN_CLUSTER_SIZE + 1):
            ...
            is_stable = (pct_spread <= pct_threshold) or (
                (cluster_max - cluster_min) <= abs_threshold
            )
    ...
    def get_stable_result(self) -> dict | None:
        ...
        return {
            ...
            "min": (
                round(self._stable_min, 2)
                if self._stable_min is not None and math.isfinite(self._stable_min)
                else self._stable_min
            ),
            "max": (
                round(self._stable_max, 2)
                if self._stable_max is not None and math.isfinite(self._stable_max)
                else self._stable_max
            ),
    ...
    def get_statistics(self) -> dict:
        ...
        return {
            ...
            "window_values": (
                list(self._window)[-5:] if self._window else []
            ),  # Last 5 for debugging
...
class BackgroundReader:
            ...
            while not self._stop_event.is_set():
                try:
                    if self.client and not self.client.is_connected():
                        ...
                        print(
                            f"[BACKGROUND_READER] Port lost, reconnecting "
                        ...
                        if reading and reading.value is not None:
                            ...
                            with self._lock:
                                ...
                                if self._stable_reading is not None and value > 0:
                                    ...
                                    relative_diff = abs(value - stable_val) / max(
                                        abs(stable_val), 1.0
                                    )
                                    if relative_diff > 0.50:
                                        ...
                                        if self._regime_change_count >= 5:
                                            ...
                                            print(
                                                f"[BACKGROUND_READER] Regime change: "
    ...
    def get_reading_with_stabilization(
        self, timeout: float = 10.0, measurement_type: str = "DC_VOLTAGE"
        ...
        """
        print(
            f"[DEBUG] get_reading_with_stabilization called: type={measurement_type}, timeout={timeout}"
        )
        ...
        while time.time() - start_time < timeout:
            ...
            if not self.is_connected():
                ...
                if not reconnected:
                    print(
                        "[BACKGROUND_READER] Disconnected during measurement wait — returning early"
                    )
                    return None
            ...
            with self._lock:
                ...
                if self._stabilizer.is_stable():
                    stable_val = self._stabilizer.get_stable_reading()
                    if (
                        stable_val is not None
                        and math.isfinite(stable_val)
                        and self._latest_reading
                    ):
