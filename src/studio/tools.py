...
def _build_thresholds_dict(config):
    ...
    for signal_id, threshold in config.thresholds.items():
        ...
        thresholds[signal_id] = {"signal_id": threshold.signal_id, "states": states}
    return thresholds
...
def query_diagnostic_knowledge(
    query: str, equipment_model: str, category: str | None = None, top_k: int = 5
) -> dict:
    """
    ...
    results = rag.retrieve(query=full_query, equipment_model=equipment_model, top_k=top_k)
...
def get_equipment_configuration(equipment_model: str, request_type: str) -> dict:
    ...
    if request_type == "test_points":
        ...
        return {
            "error": f"Unknown request_type: {request_type}. Valid types: test_points, thresholds, faults, images, all"
        }


@tool
def get_test_point_guidance(equipment_model: str, test_point_id: str) -> dict:
    """
    ...
    try:
        ...
        guidance["location_description"] = (
            f"Test point {test_point_id} location - {guidance.get('name', '')}"
        )
...
def read_multimeter(
    ...
    try:
        guidance_result = get_test_point_guidance.invoke(
            {"equipment_model": equipment_model, "test_point_id": test_point}
        )
    except Exception as e:
        guidance_result = {"error": f"Failed to get guidance: {str(e)}", "test_point": test_point}
    ...
    reading = reader.get_reading_with_stabilization(
        timeout=max_duration, measurement_type=mtype_enum
    )
...
def get_diagnostic_step(
    current_state: dict[str, Any], equipment_model: str = "cctv-psu-24w-v1"
) -> dict:
    """
    ...
    try:
        ...
        if not state.equipment_config:
            ...
            engine = DiagnosticEngine(equipment_config_loader=config_loader, state=state)
        ...
        return {"status": "error", "error": str(e), "message": "Failed to get diagnostic step"}
    ...
    try:
        ...
        if numbers:
            ...
            if "-" in expected_str and len(numbers) >= 2:
                ...
                is_normal = (
                    (expected_num - tolerance) <= measured_value <= (expected_num + tolerance)
                )

        if is_normal:
            interpretation = "normal"
            conclusion = (
                f"Measured {measured_value}{unit} is within expected range ({expected_value})"
            )
        else:
            interpretation = "abnormal"
            conclusion = (
                f"Measured {measured_value}{unit} is outside expected range ({expected_value})"
            )
...
@tool
def wait_for_multimeter_reading(test_point_id: str, timeout: int = 15) -> dict:
    """
...
def enter_manual_reading(
    test_point_id: str, value: float, unit: str, measurement_type: str = "manual"
) -> dict:
    """
