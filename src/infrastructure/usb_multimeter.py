...
class MastechMS8250DParser:
    ...
    def parse_flags(cls, buf: bytes) -> dict:
        ...
        flags = {
            ...
            "is_micro": (
                bool(buf[8] & (1 << 4)) if not (buf[10] & (1 << 1)) else bool(buf[9] & (1 << 1))
            ),  # uF has different bit
...
class USBMultimeterClient:
    ...
    def _parse_new_frame_format(self, frame: bytes) -> MultimeterReading | None:
        ...
        try:
            ...
            is_new_header = (frame[1] == 0xFE and frame[2] == 0xEC) or (
                frame[1] == 0xEE and frame[2] == 0xCC
            )
    ...
    def _parse_um24c_frame(self, frame: bytes) -> MultimeterReading | None:
        ...
        try:
            ...
            if len(frame) >= 9 and frame[0] in [0x40, 0x44]:
                ...
                # DEBUG: Log digits
                print(
                    f"[DEBUG] digit1={digit1} (0x{frame[5]:02x}), digit2={digit2} (0x{frame[6]:02x})"
                )
                ...
                if 0 <= digit1 <= 9 and 0 <= digit2 <= 9:
                    ...
                    print(
                        f"[DEBUG PARSE] frame={frame.hex().upper()}, digit1={digit1}, digit2={digit2}, decimal_pos=0x{decimal_pos:02x}"
                    )
                    ...
                    print(
                        f"[DEBUG] Calculated value={value}, mode_byte=0x{frame[3]:02x}, unit_byte=0x{frame[4]:02x}"
                    )
                    ...
                    if mode_byte == 0x75:
                        ...
                        print(
                            f"[DEBUG] Unknown mode! mode_byte=0x{mode_byte:02x}, unit_byte=0x{unit_byte:02x} - defaulting to DC_VOLTAGE"
                        )
...
def create_multimeter_client(
    port: str | None = None, on_reading_callback: Callable[[MultimeterReading], None] | None = None
) -> USBMultimeterClient:
    """
