...
def parse_measurement(arg: str) -> dict:
    ...
    return {"test_point": parts[0], "value": float(parts[1]), "unit": parts[2]}
        ...
        while time.time() - start_time < timeout:
            ...
            if reading:
                ...
                if (
                    ...
                    or reading.value != last_reading.value
                    or reading.unit != last_reading.unit
                ):
                    ...
                    if len(readings_for_avg) > 1:
                        ...
                        if len(stable_values) >= 2:
                            ...
                            print(
                                f"  [Averaged] {avg_value:.2f} {reading.unit} (from {len(stable_values)}/{len(values)} samples)"
                            )
...
def scenario_replay(scenario_file: str):
    ...
    equipment_id = (
        scenario.get("signals", [{}])[0].get("test_point", {}).get("id", "CCTV-PSU-24W-V1")
    )
...
Examples:
  ...
  python -m src.interfaces.cli --interactive
    ...
    mode_group.add_argument(
        "--status", "-S", action="store_true", help="Show current mode configuration"
    )

    # Interactive and replay (keep for backward compatibility)
    parser.add_argument(
        "--interactive", "-i", action="store_true", help="Run in interactive mode with manual input"
    )

    # Standard diagnostic arguments
    parser.add_argument("--model", "-m", type=str, help="Equipment model")
    parser.add_argument(
        "--measurements", "-V", type=str, nargs="+", help="Measurements in format TP:value:unit"
    )
    parser.add_argument("--trigger", "-t", type=str, default="", help="Trigger description")
